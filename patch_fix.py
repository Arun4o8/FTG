...
# The start and end to replace
js_start = "        // --- Dynamic Questions ---"
js_end = "        // --- Form Submission Logic ---"

new_js = r"""        // --- Dynamic Questions ---
        const questionsPool = [
            {
...
if content.find(js_start) != -1 and content.find(js_end) != -1:
    content = content[:content.find(js_start)] + new_js + content[content.find(js_end):]
    js_replacement_done = True

with open(file_path, "w") as f:
    f.write(content)

print("Patch fix applied successfully.")
